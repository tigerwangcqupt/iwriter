...
import com.yryz.writer.modules.bank.provider.BankProvider;
...
public class BankUtil {
    ...
    //传入卡号得到银行名称
    public static String getBankNameByCard(String idCard){
           String bankName = "";
           try{
               bankName = getNameOfBank(idCard);
               if(StringUtils.isNotEmpty(bankName)){
                   bankName = bankName.split("·")[0];
               }
           }catch (Exception e){
               logger.error(idCard+"转换银行卡名称失败",e);
           }
           return bankName;
    }
