...
public interface ProvinceDao {
    ...
    List<Province> selectList(ProvinceDto provinceDto);

    Integer deleteByPrimaryKey(Long id);

    Integer insert(Province province);
    
 	Integer insertByPrimaryKeySelective(Province province);
 
    Province selectByPrimaryKey(Long id);

    Integer updateByPrimaryKeySelective(Province province);
    
    List<ProvinceVo> queryAllProvinces();
}